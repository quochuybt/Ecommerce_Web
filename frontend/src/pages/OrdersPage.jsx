    ...
    <main className="min-h-screen bg-page py-10">
      <section className="mx-auto max-w-app px-6">
        <div className="mb-8">
          ...
          {error && <p className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">Không tải được đơn hàng từ backend: {error}</p>}
        </div>
        <div className="overflow-hidden rounded-xl border border-line bg-white shadow-sm">
          {orders.length === 0 ? (
            <div className="px-5 py-10 text-center text-sm text-muted">Chưa có đơn hàng.</div>
          ) : orders.map((order) => (
            <div className="grid gap-3 border-b border-line px-5 py-4 text-sm last:border-b-0 md:grid-cols-[1.2fr_1fr_1fr_1fr_auto] md:items-center" key={order.id}>
              ...
              <span className="text-[#414754]">{order.created_at ? new Date(order.created_at).toLocaleDateString("vi-VN") : ""}</span>
              <strong className="text-primary">{order.status}</strong>
              <span className="text-[#414754]">{order.items?.length || 0} sản phẩm</span>
              <strong className="text-accent">{formatPrice(order.total_amount || 0)}</strong>
            </div>
          ))}
        </div>
      </section>
