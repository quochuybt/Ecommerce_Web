...
export default function AdminDashboard() {
  ...
  const metrics = useMemo(
    () => [
      { label: "Doanh thu", value: formatPrice(dashboard?.revenue || 0), icon: DollarSign },
      { label: "Đơn hàng", value: String(dashboard?.orders_count || 0), icon: ShoppingBag },
      { label: "Sản phẩm", value: String(dashboard?.products_count || 0), icon: Package },
      { label: "Người dùng", value: String(dashboard?.users_count || 0), icon: UsersRound },
    ],
    [dashboard]
  );
    ...
    <section>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map((metric) => {
          ...
          return (
            <article className="rounded-xl border border-line bg-white p-5 shadow-sm" key={metric.label}>
              <Icon className="h-6 w-6 text-primary" />
              <span className="mt-3 block text-sm font-medium text-muted">{metric.label}</span>
              <strong className="mt-1 block text-2xl text-ink">{metric.value}</strong>
            </article>
          );
        })}
      </div>
