...
export default function ProductDetailPage() {
  ...
  if (!product) return <main className="min-h-screen bg-page px-6 py-16"><div className="mx-auto max-w-app text-2xl font-bold">Không tìm thấy sản phẩm</div></main>;
  ...
  return (
    <main className="min-h-screen bg-page py-10">
      <div className="mx-auto max-w-app px-6">
        <Link to="/products" className="mb-6 inline-flex items-center gap-2 text-sm font-semibold text-primary">
          <ChevronLeft className="h-4 w-4" /> Quay lại sản phẩm
        </Link>
        {error && <p className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">{error}. Đang dùng dữ liệu fallback.</p>}
        ...
        <section className="grid gap-8 rounded-2xl border border-line bg-white p-5 shadow-soft lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <div className="space-y-5 p-2">
            ...
            <div className="flex items-center gap-2 text-sm">
              <div className="flex">
                {[1, 2, 3, 4, 5].map((star) => <Star key={star} className={`h-4 w-4 ${star <= Math.floor(product.rating || 0) ? "fill-amber-500 text-amber-500" : "fill-gray-200 text-gray-200"}`} />)}
              </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <button onClick={() => addToCart(product)} className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-primary px-6 text-sm font-semibold text-white transition-colors hover:bg-primary-dark">
                <ShoppingCart className="h-[18px] w-[18px]" /> Thêm vào giỏ
              </button>
              <button className="inline-flex h-11 items-center justify-center gap-2 rounded-lg border border-[#c1c6d6] px-6 text-sm font-semibold text-ink transition-colors hover:bg-soft">
                <Heart className="h-[18px] w-[18px]" /> Yêu thích
              </button>
            </div>
